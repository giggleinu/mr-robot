...
const App: FunctionComponent = (): JSX.Element => {
  ...
  const processCommand = (): void => {
    ...
    const commandInputCleaned = inputCapitalisedSplit.filter((cmd) => (!!cmd))
    const inputBaseCommand = commandInputCleaned[0];
    // Errors
    const notPlaced = !isPlaced && inputBaseCommand !== BaseCommandStr.PLACE;
    const invalidBaseCommand = !Object.keys(BaseCommandStr).includes(inputBaseCommand)
    const invalidParams =
    ...
    // * * * * * * * * * * PLACE * * * * * * * * * * //
      ...
      if (commandInputCleaned.length === 4) {
        ...
        const validOrientation = Object.keys(Orientation).includes(inputFacing);
    ...
    // * * * * * * * * * * LEFT & RIGHT * * * * * * * * * * //
    else if (inputBaseCommand === BaseCommandStr.LEFT || inputBaseCommand === BaseCommandStr.RIGHT) {
      setRobotState({
        ...robotState,
        error: null,
        facing: facing && getAdjacentOrientation(facing, BaseCommand[inputBaseCommand as BaseCommand.LEFT | BaseCommand.RIGHT]),
        commands: [...robotState.commands, {
          command: BaseCommand[inputBaseCommand as BaseCommand],
        }]
      })
  ...
  return (
    <ChakraProvider theme={theme}>
      ...
      <div className="page">
        <Container>
          <Heading as="h1" size={"2xl"}>Mr Robot</Heading>
          <CommandInput
            value={commandInput}
